...
void	init_ver(t_ray *ver, t_pl pl)
{
	if (pl.dir < (float)M_PI_2 && pl.dir > (float)(M_PI * 1.5))
		...
		ver->x = (int)pl.x / SCALE * SCALE - 1;
	ver->y = (int)pl.y + (tan(pl.dir) * fabsf(ver->x - pl.x)) - 1;
	ver->s_x = SCALE;
	if (pl.dir > (float)M_PI_2 && pl.dir < (float)(M_PI * 1.5))
		ver->s_x *= -1;
	ver->s_y = tan(pl.dir) * fabsf(ver->s_x - pl.x);
	ver->len_ray = fabs((pl.x - ver->x) / cos(pl.dir));
}
...
void	init_hor(t_ray *hor, t_pl pl)
{
	...
	hor->len_ray = fabs((pl.x - hor->x) / cos(pl.dir));
}
...
void	draw_player(t_data *data)
{
	...
	while (!wall_h || !wall_v)
	{
		if (!wall_v && ver.len_ray <= hor.len_ray)
		{
			...
			ver.y += ver.s_y;
			wall_v = check_wall(&ver, data->map);
			ver.len_ray = fabs((data->pl->x - ver.x) / cos(data->pl->dir));
		}
		if (!wall_h && ver.len_ray >= hor.len_ray)
		{
			...
			hor.y += hor.s_y;
			wall_h = check_wall(&hor, data->map);
			hor.len_ray = fabs((data->pl->x - hor.x) / cos(data->pl->dir));
		}
	}
